...
// Garantisce type-safety per i dati passati al componente DeviceAction.
interface DeviceItemProps {
  label: string;
  description: string;
  icon: React.ReactNode; // Permette di passare JSX/SVG come prop
  href: string;
}
...
const Devices: React.FC = () => {
  return (
    // flex-1: Impone al contenitore di occupare lo spazio rimanente.
    // min-w-0: Hack CSS fondamentale in Flexbox per evitare il "flex overflow" 
    // (impedisce che i testi rompano la gabbia allargandola troppo).
    <section className="flex-1 min-w-0 pb-20">
      ...
      <div className="flex flex-col gap-8 mt-4">
        ...
        <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
          ...
          {/* divide-y: Utility Tailwind avanzata. 
              Inietta automaticamente un bordo divisorio tra tutti gli <li> figli. */}
          <ul className="divide-y divide-gray-100">
            <DeviceAction 
              label="Accesso e dispositivi" 
              description="Gestisci i dispositivi connessi"
              icon={<TvMobileIcon />}  // Rendering del sottomodulo SVG
          ...
          <div className="bg-white rounded-md shadow-sm border border-gray-200 overflow-hidden">
            ...
            <ul className="divide-y divide-gray-100">
              <DeviceAction 
                ...
                icon={<DownloadIcon />}  // Rendering del sottomodulo SVG
...
// Riceve le proprietà destrutturate, conformi all'interfaccia DeviceItemProps.
const DeviceAction: React.FC<DeviceItemProps> = ({ label, description, icon, href }) => (
  <li>
    {/* Uso del Link Next.js. La classe 'group' crea un contesto padre per gli stati hover dei figli. */}
    <Link href={href} className="flex items-center justify-between p-6 hover:bg-gray-50 group transition-colors">

      <div className="flex items-center gap-5">

        {/* shrink-0: Impedisce la deformazione dell'icona se c'è poco spazio orizzontale */}
        <div className="text-gray-900 w-6 flex justify-center shrink-0">
          {icon}
        </div>
        <div className="flex flex-col">
          {/* group-hover:underline: Il testo si sottolinea solo quando il contenitore <Link> va in hover */}
...
// --- ICONE SVG ORIGINALI ---
// L'estrazione delle icone in funzioni separate mantiene il JSX del blocco principale pulito.
const TvMobileIcon = () => (
