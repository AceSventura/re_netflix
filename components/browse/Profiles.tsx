...
const ProfileSelection = () => {
    const { profiles, selectProfile, isLoading, error } = useProfiles();

    if (isLoading) return <div className="min-h-screen flex items-center justify-center bg-[#141414] text-white">Caricamento...</div>;

    if (error) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-[#141414] text-white">
            <p className="text-red-500 mb-4">{error}</p>
            <button onClick={() => window.location.reload()} className="px-4 py-2 bg-white text-black rounded">Riprova</button>
        </div>
    );
            ...
            <div className="flex flex-col items-center max-w-6xl px-8">
                ...
                {profiles.map((profile) => (
                    <li key={profile.id_profilo} className="group cursor-pointer">
                        <div onClick={() => selectProfile(profile)}>
                            ...
                            <span className="mt-4 block text-center text-gray-400 group-hover:text-white text-lg md:text-xl transition-colors duration-300">
                ...
                {profiles.length < 5 && (
                    <li className="group cursor-pointer">
                        <a href="/profiles/manage" className="flex flex-col items-center">
